...
final class InsightViewModel: ObservableObject {
    ...
    private static func fallbackUsageCard(summary: monthlyUsageSummary) -> EnergyInsightCard {
        let percent = Int(summary.percentChange.rounded())
        if percent >= 0 {
            return EnergyInsightCard(
                title: "Lower Energy Usage This Week",
                bodyMessage: "Your electricity consumption dropped by \(percent)% compared to last week, saving you an estimated Rp\(abs(summary.rupiahdiff))!"
            )
        } else {
            return EnergyInsightCard(
                title: "Higher Energy Usage This Week",
                bodyMessage: "Your electricity consumption rose by \(abs(percent))% compared to last week, costing you an estimated Rp\(summary.rupiahdiff) more."
            )
        }
    }
